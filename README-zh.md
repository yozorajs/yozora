...
- 🔖 完整地支持了 [GFM 规范][gfm-spec] 中提到的所有规则，并通过了几乎所有由规范中的示例改造成的测试
  ...
  可参见 [@yozora/parser-gfm] 或 [@yozora/parser-gfm-ex] 以获得进一步信息。

- 🚀 健壮性：

  - 所有代码都采用 TypeScript 编写，拥有严格的静态类型检查保障；
  - 使用 ESLint 和 Prettier 约束编码风格，规避偏僻语法及 shadow variables 之类易于出错的问题；
  - 使用 Vitest 和大量测试用例进行测试；
...
- ⚡️ 高性能
  ...
  - 在读入字符串时，会将其预处理成字符编码及位置信息，使用 [INodePoint][src-INodePoint] 数据类型
    承载。在分词阶段通过扫描 [INodePoint][src-INodePoint] 的方式完成匹配，虽然匹配过程更麻烦些，但
    理论上比正则表达式匹配少一些常数开销；

  - 小心地处理数组新建/连接操作，整个扫描阶段尽量复用数组，仅通过下标索引来圈定匹配范围，并应用了
    不少策略减少重复匹配/解析操作。
...
- [@yozora/parser-gfm][]: 支持 [GFM 规范][gfm-spec] 的 Markdown 解析器。内置了支持 [GFM
  规范][gfm-spec] 中提到的所有语法（**不包含**规范中提到的扩展语法，如
...
- [@yozora/parser-gfm-ex][]: 支持 [GFM 规范][gfm-spec] 的 Markdown 解析器。内置了支持 [GFM
  规范][gfm-spec] 中提到的所有语法（**包括**规范中提到的扩展语法，如
  [table][@yozora/tokenizer-table]）的分词器。
