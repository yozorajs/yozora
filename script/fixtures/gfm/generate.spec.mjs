...
test('is byte-idempotent and preserves all checked-in answers', async t => {
  ...
  assert.deepEqual(summary, { currentExamples: 677, matched: 671, new: 6, old: 1 })
  assert.deepEqual(after, before)

  const mainCases = Object.entries(after.gfm)
    .filter(([filename]) => /^#\d{3}[.]json$/.test(filename))
    .map(([, raw]) => JSON.parse(raw).cases[0])
  assert.equal(mainCases.filter(fixtureCase => 'parseAnswer' in fixtureCase).length, 671)
  assert.equal(mainCases.filter(fixtureCase => 'markupAnswer' in fixtureCase).length, 653)
})
...
test('validates the checked-in partition and pinned source metadata', () => {
  ...
  assert.equal(currentIds.length, 671)
  assert.deepEqual(newIds, ['#491', '#493', '#633', '#634', '#635', '#657'])
