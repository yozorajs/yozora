 ...
 * 内联数据的分词器的上下文
 */
export class InlineDataNodeTokenizerContext
  ...
  public constructor(
    ...
    for (const t of fallbackTokenizer.acceptedTypes) {
  ...
  protected deepParse(
    ...
    const self = this
    const results: DataNode<DataNodeCategory.INLINE, InlineDataNodeType>[] = []
    for (const p of tokenPositions) {
      ...
      if (p.children != null && p.children.length > 0) {
        children = self.deepParse(content, codePoints,
          p.children as DataNodeTokenPosition<InlineDataNodeType>[])
      }
      const result: DataNode = tokenizer.parse(content, codePoints, p, children)
      results.push(result as DataNode<DataNodeCategory.INLINE, InlineDataNodeType>)
    }
    return results
  }
