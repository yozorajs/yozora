...
- `onClose`: (Optional) Called before the current node closes to perform cleanup. A
  `failedAndRollback` result may include `remainingTokens` to retain matched children after the
  rolled-back lines, before parent containers inspect their block structure.
